...
# def updateRecord(records, prov1, prov2):
#     # for record in records:
#     if len(records['QDS']) <=6:
#         record['Check'] = 'y'
#     elif prov1 == prov2: #if they match just update
#             record['updatedProv'] = prov1   #add updated province
#             record['Check'] = ''
           
#     else: #if province from QDS only, record and mark to check
#         if prov1 or prov2 is None:
#             if prov2 is None:
#                 record['updatedProv'] = prov1
#                 record['Check'] = 'y' 
#             else: #if province from town only, then if exact match use the province, else use the province and mark for check
